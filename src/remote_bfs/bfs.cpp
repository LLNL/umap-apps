...
static uint16_t bfs_k_infinite_level = std::numeric_limits<uint16_t>::max();
...
#pragma omp parallel
  ...
  std::cout << "OpenMP schedule = " << disp_omp_schedule(kind)
...
#pragma omp parallel for schedule (runtime)
    for (uint64_t src = 0; src < num_vertices; ++src) { /// BFS loop for each level
      ...
      for (size_t i = index[src]; i < index[src + 1]; ++i) {
        ...
        if (!utility::get_bit(visited_filter, trg) && level[trg] == bfs_k_infinite_level) {
          level[trg] = current_level + 1;
          utility::set_bit(visited_filter, trg);
          visited_new_vertex = true;
        }
      }
    } /// End of BFS loop for each level  
		 ...
		 const uint16_t *const level)
{
  ...
  for (uint64_t i = 0; i < num_vertices; ++i) {
    if (level[i] == bfs_k_infinite_level) continue;
